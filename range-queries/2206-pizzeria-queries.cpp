...
struct Node {
    long long sum, mx, mn, len, pref, suff;
    
    Node () : sum(0), mx(-1e17), mn(1e17), pref(1e17), suff(1e17), len(0) {}
    Node (long long val) : sum(val), mx(val), mn(val), pref(val), suff(val), len(1) {}
    ...
    void merge (Node &left, Node &right) {
		...
		pref = min(left.pref, left.len + right.pref);
		suff = min(right.suff, right.len + left.suff);
    }
};
...
int main() {
	...
	while (q--) {
		...
		if (type == 1) {
			...
			long long left = sg.query(0, index).suff;
			long long right = sg.query(index, n - 1).pref;
